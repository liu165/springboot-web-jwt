...
public class JwtFilter implements Filter {
    ...
    public JwtFilter(TokenProvider tokenProvider, String[] authorisedUrls) {
        ...
        try {
            ...
            for (String url : urls) {
                if (pathMatcher.match(url, spath)) {
                    boolean token = tokenProvider.validateToken(httpRequest.getHeader("token"));
                    if (token) {
